...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("zencoinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("zencoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zencoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running zencoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
